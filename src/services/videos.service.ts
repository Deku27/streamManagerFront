...
export class VideosService {
  ...
  getVideosInfo() {
    return this.http
      .get('http://localhost:8080/getVideosInfo')
      .pipe(map(res => res));
  }

  getVideoByID(id: number) {
    return this.http
      .get('http://localhost:8080/video/' + id)
      .pipe(map(res => <Video>res));
  }

  saveVideo(video: Video) {
    return this.http
      .post('http://localhost:8080/videos', video)
      .pipe(map(res => res));
  }

  editVideo(id: number, video: Video) {
    return this.http
      .put('http://localhost:8080/video/' + id, video)
      .pipe(map(res => res));
  }

  deleteVideo(id: number) {
    return this.http
      .delete('http://localhost:8080/video/' + id)
      .pipe(map(res => res));
  }
